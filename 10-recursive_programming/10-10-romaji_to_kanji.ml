...
(* 10-10 *)
(* purpose : ローマ字表記の駅名と駅名リストを受け取ると漢字表記の駅名を返す *)
(* romaji_to_kanji : string -> ekimei_t list -> string *)
let rec romaji_to_kanji ekimei lst = match lst with
  [] -> ""
  | ({kanji= k;romaji = r;})::rest ->
    if r = ekimei then k else romaji_to_kanji ekimei rest

let test1 = romaji_to_kanji ""            global_ekimei_list = ""
let test2 = romaji_to_kanji "toyosu"      global_ekimei_list = "豊洲"
